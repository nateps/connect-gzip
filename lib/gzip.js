 ...
 *  - `matchFile`   Regular expression matching filename to be compressed
...
exports = module.exports = function gzip(options) {
  ...
  if (!matchFile.test) throw new Error('option matchFile must be a regular expression');
  ...
  return function gzip(req, res, next) {
    ...
    res.writeHead = function(code) {
      ...
      if (req.method === 'HEAD' || code !== 200 || !~accept.indexOf('gzip') ||
          (!matchType.test(type) && !matchFile.test(filename)) || encoding ||
